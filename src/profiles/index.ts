...
import MinimumProfileConverter, { TaxIdentifierType } from './minimum/minimum';
...
export type XMLSchemeNames = "MINIMUM_XML" | "BASICWL_XML" | "BASIC_XML" | "EN16931_XML" | "EXTENDED_XML" | "XRECHNUNG_XML";
...
abstract class Converter<XMLScheme extends object, ParsedObjectScheme extends object> {
    ...
    set xml(xml: XMLScheme) {
        if (!this.isProperXMLScheme(xml)) throw new Error("Somethings wrong with the Data you provided: Scheme does not fit expected Scheme");
    ...
    set invoice(invoice: ParsedObjectScheme) {
        if (!this.isProperObjectScheme(invoice)) throw new Error("Somethings wrong with the Data you provided: Scheme does not fit expected Scheme");
    ...
    private obj2xml(obj: ParsedObjectScheme): XMLScheme {
        ...
        if (!xml['rsm:CrossIndustryInvoice']) throw new Error("Conversion from XML to Obj failed! No rsm:CrossIndustryInvoice");
        xml = {
    ...
    private obj2xmlTypeConverter(objValue: any, sourceType: ObjectTypes | "Array", arrayMap?: MappingItem[]): any {
        ...
        switch (sourceType) {
            ...
            case "number": {
                ...
                let value = objValue.toString();
                return value;
            }
            case "number_decimal_2": {
                if (isNaN(objValue)) throw new DatatypeValidationError(sourceType, objValue.toString());
                let value = objValue.toFixed(2);
                return value;
            }
            case "Date":
    ...
    private cleanObject(obj: any): any {
        ...
        */
        ...
        let cleanedObj: any = Array.isArray(obj) ? [] : {};
    ...
    private mapTaxIdFromXml2Obj(xmlValue: any): TaxIdentifierType {
        ...
        if (!(schemeID === "VA") && !(schemeID === "FC")) throw new DatatypeValidationError("taxid", xmlValue.toString());
